def SMO_Platt(inputData, inputLabels, C = 0.6, toler = 0.001, maxIter = 40, kPara = ('Lin',0)):
    ...
    oS = optStruct(inputData, inputLabels, C, toler)
    numOfIter = 0
    entireSet = True
    alphaPairsChanged = 0
    ...
    while((numOfIter < maxIter) and (alphaPairsChanged > 0 ) or (entireSet)):
        alphaPairsChanged = 0
        ...
        if entireSet:
            ...
            notInBound = list(nonzero((oS.alphas.A > 0) * (oS.alphas.A < C)))[0]
        ...
        weight =mat(calcWs(oS.alphas , inputData, inputLabels))
    return weight , oS.b